...
async function login(req, res, next) {
    ...
    switch (role) {
        ...
        case 'doctor': user = await Doctor.findOne({ email }); break;
        case 'nurse': user = await Nurse.findOne({ email }); break;
        default: return next(new CustomError('Invalid role', 400));
    }

    if (!user)
        return next(new CustomError(`No ${role} with this email ${email}`, 404));
